...
The vaccinated group has a lower mean infectious period, but a larger variance. The difference is not found in the seeder group, but is due to differences in the contact group. 
...
mean.R0 <- data.frame(Vaccinated = c("No","Yes"),
                      ...
                      infper = mean.inf.per[mean.inf.per[,2]=="contact",]$x,
                      llinfper =  mean.inf.per[mean.inf.per[,2]=="contact",]$x+qnorm(alpha/2,lower.tail = T)*sem.inf.per[sem.inf.per[,2]=="contact",]$x,
                      ulinfper =  mean.inf.per[mean.inf.per[,2]=="contact",]$x+qnorm(alpha/2,lower.tail = F)*sem.inf.per[sem.inf.per[,2]=="contact",]$x,
                      logR0 = c(mean.logbeta[1]  ,sum(mean.logbeta))+mean.log.inf.per[mean.inf.per[,2]=="contact",]$x,
                      lllogR0 = (c(mean.logbeta[1]  ,sum(mean.logbeta))+mean.log.inf.per[mean.inf.per[,2]=="contact",]$x) + qnorm(alpha/2,lower.tail = T)*(se.logbeta + sem.log.inf.per[sem.inf.per[,2]=="contact",]$x),
                      ullogR0 = (c(mean.logbeta[1]  ,sum(mean.logbeta))+mean.log.inf.per[mean.inf.per[,2]=="contact",]$x) + qnorm(alpha/2,lower.tail = F)*(se.logbeta + sem.log.inf.per[sem.inf.per[,2]=="contact",]$x)
                      )
...
alpha = 0.05 #conf.level
#beta
mean.logbeta <- summary(fit.empty)$coefficients[,1] 
se.logbeta<- summary(fit.empty)$coefficients[,2]
...
#R0
mean.R0 <- data.frame(Vaccinated = c("No"),
                      beta = exp(c(mean.logbeta[1],sum(mean.logbeta))),
                      llbeta = exp(profile.confint[1]+c(mean.logbeta[1],sum(mean.logbeta))),
                      ulbeta = exp(profile.confint[2]+c(mean.logbeta[1],sum(mean.logbeta))),
...
ggplot(data = all.results) + 
  geom_point(aes(x = Assumption, y =infper))+
  geom_errorbar(aes( x = Assumption, ymin = llinfper, ymax = ulinfper))+
