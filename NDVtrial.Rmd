...
#R0
mean.R0 <- data.frame(Vaccinated = c("No","Yes"),
                      beta = exp(c(mean.logbeta[1])),
                      llbeta = exp(profile.confint[1]+0),
                      ulbeta = exp(profile.confint[2]+0),
                      infper = mean.inf.per$x,
...
all.results <- all.results[c(1,2,5,6,7,8,9,10),]
```
## Infectious period
```{r}
ggplot(data = all.results) + 
  geom_point(aes(x = Assumption, y =infper,colour = Assumption ))+
...
```{r}
ggplot(data = all.results) + 
  geom_point(aes(x = Assumption, y = beta,colour = Assumption ))+
...
```{r}
ggplot(data = all.results) + 
  geom_point(aes(x = Assumption, y = R,colour = Assumption ))+
