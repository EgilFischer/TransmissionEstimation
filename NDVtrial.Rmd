...
Use the function ``` reform.data ``` to rearrange the data for analyses. The function takes the data and use the names of the sampledays, determine positivity based on one or all positive samples, the underlying transmission model (SI, SIR, SIS), and define 'infection' and 'recovery' based on the number of consecutive positive or negative samples and define the cutt-off value for positivity. 
For the basic analyses the following values are used:
- cut-off Ct-value 36
- SIR-model
- Chicken positive if positive on 2 consecutive days
- Chicken negative if negative on 2 consecutive days
Three different scenarios were tested:
- One or both ON/CLO sample positive
- ON sample positive
- CLO sample positive
...
epicurveplot <- ggplot(data = cbind(data.one.positive[[1]], pos = "At least one sample positive" )) +
...
survivalplot <- ggplot(data = data.one.positive[[1]]) +
...
Analysis based on ON-swabs or when at least one is positive is recommended.
...
out.indiv$infper <- apply(data.one.positive[[2]][,c(5:18)]==1, 1, sum, na.rm = T);
out.indiv[out.indiv$infper == 0,]$infper<- NA #zero means never been infected
```
...
overall <- cbind(mean(out.indiv$infper,na.rm = T), sd(out.indiv$infper,na.rm = T))
mean <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),mean,na.rm = T)
sd <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),sd,na.rm = T)
...
#descriptive statistics overall
overall <- cbind(mean(out.indiv$infper,na.rm=T), sd(out.indiv$infper,na.rm=T))
mean <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated, out.indiv$Challenge),mean,na.rm=T)
sd <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated, out.indiv$Challenge),sd,na.rm=T)
...
```{r, include = F}
profile.confint <- confint(fit.full);
#beta's
...
#infectious period
mean.inf.per<- aggregate(out.indiv$infper, list(out.indiv$Vaccinated, out.indiv$Challenge),mean, na.rm = T)
sem.inf.per <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated,  out.indiv$Challenge),sem)
mean.log.inf.per <- aggregate(log(sapply(out.indiv$infper,function(x){max(x,.5)})), list(out.indiv$Vaccinated, out.indiv$Challenge),mean, na.rm = T)
...
mean.R0 <- data.frame(Vaccinated = c("No","Yes"),
                      ...
                      llbeta = exp(profile.confint["out.nona$VaccinatedYes",1]+c(0,mean.logbeta[1])),
...
out.indiv$infper <- apply(data.ON.positive[[2]][,c(5:18)]==1, 1,sum, na.rm = T);
out.indiv$infper[out.indiv$infper==0] <- NA;
...
overall <- cbind(mean(out.indiv$infper,na.rm=T), sd(out.indiv$infper,na.rm=T), sem(out.indiv$infper))
mean <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),mean,na.rm=T)
sd <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),sd,na.rm=T)
sem <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),sem)
group <- c("Overall", "Unvaccinated", "Vaccinated")
table.infper <- data.frame(Group = group, 
                           Mean = rbind(overall[1,1],
                                        mean[mean$Group.1 =="No",2],
                           ...
                           SD = rbind(overall[1,2],sd[sd$Group.1 =="No",2],
                                      sd[sd$Group.1 =="Yes",2] ),
                           SEM = rbind(overall[1,3],sem[sem$Group.1 =="No",2],
                                      sem[sem$Group.1 =="Yes",2]))
```
Descriptive statistics for the infectious period.
```{r}
table.infper
...
#infectious period
mean.inf.per<- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),mean,na.rm = T)
sem.inf.per <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),sd,na.rm = T)$x/sqrt(c(sum(out.indiv$Vaccinated =="No"), sum(out.indiv$Vaccinated =="Yes")))
mean.log.inf.per <- aggregate(log(sapply(out.indiv$infper,function(x){max(x,.5)})), list(out.indiv$Vaccinated),mean,na.rm = T)
sem.log.inf.per<- aggregate(log(sapply(out.indiv$infper,function(x){max(x,.5)})),
                            list(out.indiv$Vaccinated),sem)
#R0
mean.R0 <- data.frame(Vaccinated = c("No","Yes"),
                      beta = exp(c(mean.logbeta[1],sum(mean.logbeta,na.rm = T))),
                      ...
                      logR0 = c(mean.logbeta[1]  ,sum(mean.logbeta,na.rm = T))+mean.log.inf.per$x,
                      lllogR0 = (c(mean.logbeta[1]  ,sum(mean.logbeta,na.rm = T))+mean.log.inf.per$x) + qnorm(alpha/2,lower.tail = T)*(se.logbeta + sem.log.inf.per$x),
                      ullogR0 = (c(mean.logbeta[1]  ,sum(mean.logbeta,na.rm = T))+mean.log.inf.per$x) + qnorm(alpha/2,lower.tail = F)*(se.logbeta + sem.log.inf.per$x)
                      )
...
#infectious period
mean.inf.per<- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),mean,na.rm = T)
sem.inf.per <- aggregate(out.indiv$infper, list(out.indiv$Vaccinated),sem)$x
mean.log.inf.per <- aggregate(log(sapply(out.indiv$infper,function(x){max(x,.5)})), list(out.indiv$Vaccinated),mean,na.rm = T)
sem.log.inf.per<- aggregate(log(sapply(out.indiv$infper,function(x){max(x,.5)})), list(out.indiv$Vaccinated),sem)$x
#R0
mean.R0 <- data.frame(Vaccinated = c("No","Yes"),
                      beta = exp(c(mean.logbeta[1],sum(mean.logbeta))),
                      llbeta = exp(profile.confint[,1]+c(0,mean.logbeta[1])),
...
#f(length(all.results[,1]) == 12){all.results <- all.results[c(1,2,3,4,5,7,9,10,11,12),]}
...
#Supplementary 
## Chicken positive if both samples are positive
Using the definition that both sample need to be positive to results in no positive days for the vaccine groups. 
```{r}
...
epicurveplot <- ggplot(data = cbind(data.all.positive[[1]], pos = "All samples positive")) +
  geom_path(aes(x = ndpch, y = I/N, colour = Vaccinated, group = Group))+
  geom_point(aes(x = ndpch, y = I/N, shape =  Group))+
  xlab("Day post inoculation")+ylab("Prevalence (I/N)")
...
epicurveplot
```
This assumption is clearly to strict and further assessment is not needed.
