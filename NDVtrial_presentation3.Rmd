...
cevadata.CT <- read.xlsx(paste0(data.dir, "DataNDVtrial_manual_Egil 20200317_Timea20200319.xlsx"),
                        sheet = "RAW (4)")
...
ggsave("InfectiousPeriodCEVAv2.tiff",hist.one.chal.v2)
```
```{r}
hist.one.chal.v2
```
...
#test seed - contact per treatment
wilcox.test(formula = AUC ~ Challenge,data = ct.data[ct.data$Sample == "ON swab"&ct.data$Vaccinated =="Yes",])
wilcox.test(formula = AUC ~ Challenge,data = ct.data[ct.data$Sample == "ON swab"&ct.data$Vaccinated =="No",])
...
#test seed - contact per treatment
wilcox.test(formula = AUC ~ Challenge,data = ct.data[ct.data$Sample == "CLO swab"&ct.data$Vaccinated =="Yes",])
wilcox.test(formula = AUC ~ Challenge,data = ct.data[ct.data$Sample == "CLO swab"&ct.data$Vaccinated =="No",])
#test vaccinated versus unvaccinated stratified by seeder-contact
